    ...
    if request.method == "POST":
        ...
        if files:
            ...
            for file in files:
                ...
                # 创建完整的保存路径
                if (
                    dir_part and dir_part != root_folder_name
                ):  # 确保目录部分不是根文件夹本身
                    # 如果有目录结构，保持原结构
                    ...
                    file_path = os.path.join(
                        file_save_dir, os.path.basename(relative_path)
                    )
                else:
    ...
    if request.method == "POST":
        try:
            ...
            if user_input:
                ...
                return JsonResponse(
                    {"received_input": user_input, "message": "输入内容已成功接收"}
                )
        ...
        try:
            ...
            if file_path:
                ...
                if os.path.exists(original_file_path):
                    ...
                    # 生成地图文件
                    ret, e, generated_files = tomap.generate_map(
                        "file", original_file_path, new_file_path, data, user_ip
                    )
            ...
            # 处理文件夹复制和所有文件后缀修改
                ...
                if original_folder:
                    ...
                    # 生成地图文件
                    ret, e, generated_files = tomap.generate_map(
                        "folder", original_folder_path, new_folder_path, data, user_ip
                    )
