...
def read_gaode_line_config(data):
    ...
    :return: 高德地图line配置字典,{'start':'','end':''}
    '''
    try:
        config_file=os.path.join(settings.BASE_DIR, 'fileapp', 'templates', 'gaode_line')
        map_config={}
...
def to_gaode_line(map_config,map_data,new_file_path):
    '''
    生成高德地图marker html文件
    ...
    try:
        ...
        for i,item in enumerate(map_data):
            line_data += f"[{item['lon']},{item['lat']}],\n"
        
        with open(new_file_path,'w', encoding='utf-8') as f:
            f.write(map_config['start']+settings.GAODE_API_KEY+map_config['mid']+line_data+map_config['end'])
    except Exception as e:
        print(e)
        return (False,e)
