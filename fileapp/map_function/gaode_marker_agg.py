...
def read_gaode_marker_agg_config(data):
    '''
    读取高德地图marker配置文件
    ...
    '''
    try:
        config_file=os.path.join(settings.BASE_DIR, 'fileapp', 'templates', 'gaode_marker_agg')
        map_config={}
...
def to_gaode_marker_agg(map_config,map_data,new_file_path):
    '''
    生成高德地图marker html文件

    :param map_config: 高德地图marker配置字典,{'start':'','end':''}
    :param map_data: 地图数据,{'lat':lat,'lon':lon,'weight_column':weight_column}
    ...
    try:
        ...
        for i,item in enumerate(map_data):
            if 'weight_column' not in item:
                item.setdefault('weight_column',1)
            marker_data += f"{'{'}weight: {item['weight_column']},lnglat:[{item['lon']},{item['lat']}]{'}'},\n"
